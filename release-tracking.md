...
| Xan | `windows-arm64-release` | Windows ARM64 MSVC; Linux ARM64 musl | Windows target still running; the initial musl job exposed a missing linker, and its corrected run is in progress | Await both jobs, then retain the exact setup or document a blocker. |
| Restic | `windows-arm64-release` | Windows ARM64 | [#3596](https://github.com/restic/restic/issues/3596) identifies Windows ARM VSS/COM work; fork matrix is queued with fail-fast disabled | Await the Windows ARM job separately from the expected secretless Linux-cloud failure. |
